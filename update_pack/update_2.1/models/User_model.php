...
class User_model extends CI_Model {
    ...
    public function get_Advisor($id = 0) {
        if ($id > 0) {
            return $this->db->get_all_user($id);
        }else {
            if ($this->check_if_Advisor_exists()) {
    ...
    public function check_if_Advisor_exists() {
    ...
    public function get_Advisor_list() {
        ...
        foreach ($query1 as $row1) {
            if (!in_array($row1['user_id'], $Advisor_ids) && $row1['user_id'] != "") {
                array_push($Advisor_ids, $row1['user_id']);
            }
        }
        if (count($Advisor_ids) > 0) {
            $this->db->where_in('id', $Advisor_ids);
    ...
    public function update_Advisor_paypal_settings($user_id = '') {
        // Update paypal keys
        $paypal_info = array();
        $paypal['production_client_id'] = html_escape($this->input->post('paypal_client_id'));
        array_push($paypal_info, $paypal);
    ...
    public function update_Advisor_stripe_settings($user_id = '') {
        // Update Stripe keys
        $stripe_info = array();
        $stripe_keys = array(
            'public_live_key' => html_escape($this->input->post('stripe_public_key')),
