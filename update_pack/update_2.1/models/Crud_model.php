...
class Crud_model extends CI_Model {
    ...
    public function all_enrolled_trainee() {
    ...
    public function get_revenue_by_user_type($timestamp_start = "", $timestamp_end = "", $revenue_type = "") {
        ...
        }elseif ($revenue_type == 'Advisor_revenue') {
            $this->db->where('user_id !=', $admin_details['id']);
            $this->db->select('id');
    ...
    public function get_Advisor_revenue($timestamp_start = "", $timestamp_end = "") {
        $course_ids = array();
        $courses    = array();
    ...
    public function update_Advisor_payment_status($payment_id = "") {
        $updater = array(
            'Advisor_payment_status' => 1
        );
    ...
    public function update_system_settings() {
        ...
        $data['value'] = html_escape($this->input->post('trainee_email_verification'));
        $this->db->where('key', 'trainee_email_verification');
        $this->db->update('settings', $data);
    }
    ...
    public function update_Advisor_settings() {
        $data['value'] = html_escape($this->input->post('allow_Advisor'));
        $this->db->where('key', 'allow_Advisor');
        $this->db->update('settings', $data);

        $data['value'] = html_escape($this->input->post('Advisor_revenue'));
        $this->db->where('key', 'Advisor_revenue');
        $this->db->update('settings', $data);
    }
    ...
    public function get_status_wise_courses_for_Advisor($status = "") {
        if ($status != "") {
    ...
    public function get_Advisor_wise_courses($Advisor_id = "", $return_as = "") {
        $courses = $this->db->get_where('course', array('user_id' => $Advisor_id));
    ...
    public function get_Advisor_wise_payment_history($Advisor_id = "") {
        $courses = $this->get_Advisor_wise_courses($Advisor_id, 'simple_array');
        if (sizeof($courses) > 0) {
    ...
    public function enrol_trainee($user_id){
    ...
    public function enrol_a_trainee_manually() {
        $data['course_id'] = $this->input->post('course_id');
        $data['user_id']   = $this->input->post('user_id');
        if ($this->db->get_where('enrol', $data)->num_rows() > 0) {
            $this->session->set_flashdata('error_message', get_phrase('trainee_has_already_been_enrolled_to_this_course'));
        }else {
            $data['date_added'] = strtotime(date('D, d-M-Y'));
            $this->db->insert('enrol', $data);
            $this->session->set_flashdata('flash_message', get_phrase('trainee_has_been_enrolled_to_that_course'));
        }
    }
    ...
    public function course_purchase($user_id, $method, $amount_paid) {
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if (get_user_role('role_id', $course_details['user_id']) == 1) {
                ...
                $data['Advisor_revenue'] = 0;
                $data['Advisor_payment_status'] = 1;
            }else {
                if (get_settings('allow_Advisor') == 1) {
                    $Advisor_revenue_percentage = get_settings('Advisor_revenue');
                    $data['Advisor_revenue'] = ceil(($data['amount'] * $Advisor_revenue_percentage) / 100);
                    $data['admin_revenue'] = $data['amount'] - $data['Advisor_revenue'];
                }else {
                    $data['Advisor_revenue'] = 0;
                    $data['admin_revenue'] = $data['amount'];
                }
                $data['Advisor_payment_status'] = 0;
            }
            $data['date_added'] = strtotime(date('D, d-M-Y'));
    ...
    public function get_Advisor_wise_course_ratings($Advisor_id = "", $ratable_type = "", $is_sum = false) {
        $course_ids = $this->get_Advisor_wise_courses($Advisor_id, 'simple_array');
        if ($is_sum) {
    ...
    function curl_request($code = '') {
        ...
        public function get_courses($category_id = "", $sub_category_id = "", $Advisor_id = 0) {
            if ($category_id > 0 && $sub_category_id > 0 && $Advisor_id > 0) {
                return $this->db->get_where('course', array('category_id' => $category_id, 'sub_category_id' => $sub_category_id, 'user_id' => $Advisor_id));
            }elseif ($category_id > 0 && $sub_category_id > 0 && $Advisor_id == 0) {
        ...
        public function filter_course_for_backend($category_id, $Advisor_id, $price, $status) {
            if ($category_id != "all") {
            ...
            if ($Advisor_id != "all") {
                $this->db->where('user_id', $Advisor_id);
            }
        ...
        public function get_free_and_paid_courses($price_status = "", $Advisor_id = "") {
            $this->db->where('status', 'active');
