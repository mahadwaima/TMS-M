...
class Admin extends CI_Controller {
  ...
  public function user_form($param1 = "", $param2 = "") {
    ...
    if ($param1 == 'add_user_form') {
      ...
      $page_data['page_title'] = get_phrase('trainee_add');
      $this->load->view('backend/index', $page_data);
    }
    elseif ($param1 == 'edit_user_form') {
      $page_data['page_name'] = 'user_edit';
      $page_data['user_id'] = $param2;
      $page_data['page_title'] = get_phrase('trainee_edit');
      $this->load->view('backend/index', $page_data);
    }
  }
  ...
  public function enrol_trainee($param1 = "") {
    if ($this->session->userdata('admin_login') != true) {
      redirect(site_url('login'), 'refresh');
    }
    if ($param1 == 'enrol') {
      $this->crud_model->enrol_a_trainee_manually();
      redirect(site_url('admin/enrol_history'), 'refresh');
    }
    $page_data['page_name'] = 'enrol_trainee';
    $page_data['page_title'] = get_phrase('enrol_a_trainee');
    $this->load->view('backend/index', $page_data);
  }
  ...
  public function Advisor_revenue($param1 = "") {
    if ($this->session->userdata('admin_login') != true) {
      redirect(site_url('login'), 'refresh');
    }
    ...
    $page_data['page_name'] = 'Advisor_revenue';
    $page_data['payment_history'] = $this->crud_model->get_revenue_by_user_type($page_data['timestamp_start'], $page_data['timestamp_end'], 'Advisor_revenue');
    $page_data['page_title'] = get_phrase('Advisor_revenue');
    $this->load->view('backend/index', $page_data);
  }
  ...
  public function Advisor_settings($param1 = "") {
    if ($this->session->userdata('admin_login') != true) {
      redirect(site_url('login'), 'refresh');
    }
    if ($param1 == 'update') {
      $this->crud_model->update_Advisor_settings();
      $this->session->set_flashdata('flash_message', get_phrase('Advisor_settings_updated'));
      redirect(site_url('admin/Advisor_settings'), 'refresh');
    }

    $page_data['page_name'] = 'Advisor_settings';
    $page_data['page_title'] = get_phrase('Advisor_settings');
    $this->load->view('backend/index', $page_data);
  }
  ...
  public function courses() {
    ...
    $page_data['selected_Advisor_id'] = isset($_GET['Advisor_id']) ? $_GET['Advisor_id'] : "all";
    $page_data['selected_price']         = isset($_GET['price']) ? $_GET['price'] : "all";
    $page_data['selected_status']        = isset($_GET['status']) ? $_GET['status'] : "all";
    $page_data['courses']                = $this->crud_model->filter_course_for_backend($page_data['selected_category_id'], $page_data['selected_Advisor_id'], $page_data['selected_price'], $page_data['selected_status']);
    $page_data['status_wise_courses']    = $this->crud_model->get_status_wise_courses();
    $page_data['Advisors']            = $this->user_model->get_Advisor();
  ...
  public function change_course_status($updated_status = "") {
    ...
    redirect(site_url('admin/courses?category_id='.$category_id.'&status='.$status.'&Advisor_id='.$Advisor_id.'&price='.$price), 'refresh');
  }

  public function change_course_status_for_admin($updated_status = "", $course_id = "", $category_id = "", $status = "", $Advisor_id = "", $price = "") {
    $this->crud_model->change_course_status($updated_status, $course_id);
    $this->session->set_flashdata('flash_message', get_phrase('course_status_updated'));
    redirect(site_url('admin/courses?category_id='.$category_id.'&status='.$status.'&Advisor_id='.$Advisor_id.'&price='.$price), 'refresh');
  }
  ...
  public function paypal_checkout_for_Advisor_revenue() {
    if ($this->session->userdata('admin_login') != 1)
    redirect(site_url('login'), 'refresh');
    ...
    $page_data['Advisor_name']       = $this->input->post('Advisor_name');
    $page_data['production_client_id']  = $this->input->post('production_client_id');
    $this->load->view('backend/admin/paypal_checkout_for_Advisor_revenue', $page_data);
  }

  public function stripe_checkout_for_Advisor_revenue() {
    if ($this->session->userdata('admin_login') != 1)
    redirect(site_url('login'), 'refresh');
    ...
    $this->load->view('backend/admin/stripe_checkout_for_Advisor_revenue', $page_data);
  }
  ...
  public function payment_success($payment_type = "", $payment_id = "") {
    ...
    if ($payment_type == 'stripe') {
      ...
      $Advisor_id = $payment_details['user_id'];
      $Advisor_data = $this->db->get_where('users', array('id' => $Advisor_id))->row_array();
      $stripe_keys = json_decode($Advisor_data['stripe_keys'], true);
      $this->payment_model->stripe_payment($token_id, $this->session->userdata('user_id'), $payment_details['Advisor_revenue'], $stripe_keys[0]['secret_live_key']);
    }
    $this->crud_model->update_Advisor_payment_status($payment_id);
    $this->session->set_flashdata('flash_message', get_phrase('Advisor_payment_has_been_done'));
    redirect(site_url('admin/Advisor_revenue'), 'refresh');
  }
